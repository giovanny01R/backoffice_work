...
export class SolicitudesContratosComponent implements OnInit {
  ...
  cambiarModo(nuevoModo: string) {
    ...
    // Asignamos el valor correcto para el Backend
    this.solicitud.tipo_solicitud = nuevoModo === 'vendedor' ? 'VENDEDOR' : 'PUNTO_VENTA';
    this.limpiarResultados();
    console.log("Modo cambiado a:", this.solicitud.tipo_solicitud);
  }

  onZonaChange() {
    const zona = this.solicitud.zona;
    if (this.dataZonas[zona]) {
      this.gruposVentaDisponibles = Object.keys(this.dataZonas[zona]);
      this.cecosDisponibles = this.relacionZonasCosto[zona] || [];
    }
  ...
  onLiquidacionChange() {
    ...
    if (zona && grupoVenta && liquidacion) {
      ...
      this.planesVirtualesOpciones = Array.isArray(final.vr) ? final.vr : [final.vr];
      this.solicitud.planVirtual = Array.isArray(final.vr) ? '' : final.vr;
    }
  }
  ...
  enviarSolicitud() {
    if (!this.solicitud.documento || this.solicitud.planVirtual === '---') {
      alert("⚠️ Datos incompletos");
      return;
    }
    this.sgpService.crearSolicitud(this.solicitud).subscribe({
      next: (res) => alert("✅ Enviado con éxito"),
